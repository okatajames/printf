 ...
 * by James and Aggrey
 * @c: char types
 * @buffer: buffer array to hanlde the print
...
{
    /* char is stored at left ad paddind the the buffers right */
    int i = 0;
    char padd = ' ';
    ...
    if (width > 1)
    {
        ...
        if (flags & F_MINUS)
        {
            return (write(1, &buffer[0], 1) + wirte(1, &buffer[BUFF_SIZE - i -1], width -1));
        }
        else
        {
...
int write_number(int is_negative, int ind, char buffer[], int flags, width, int precision, int size)
{
    ...
    {
        {
            ...
            else if (!(flags & F_MINUS) && padd == ' ') /* add extra char to the left of buff */
            {
                if (extra_c)
                {
                    buffer[--ind] = extra_c;
 ...
 * @is_negative: number that shows that the num is negative
 * @ind: index at which the number starts in the buffer
 * @buffer: an array of chars
 * @flags: flags specifiers
 * @width: width specifier
 * @precision: precision specififer
 * @size: size specifier
...
int write_unsgnd(int is_negative, int ind, char buffer[], int flags, int width, int precision, int size)
{
    ...
    if (precision > 0 && precision < length)
   {
    ...
    if (width > length)
    {
        ...
        buffer[i] = '\0';
        if (flags & F_MINUS) /* assign an extra char to the left of the buffer [buffer > padd] */
        {
            return (write(1, &buffer[ind], length) + wirte(1, &buffer[0], i));
        }
        else /*Assign extra char to legt of padding [padd > buffer] */
        {
...
int write_pointer (char buffer[], int ind, int length, int width, int flags, char padd, char extra_c, int padd_start)
{
    ...
    if (width > length)
    {
        ...
        buffer[i] = '\0';
        if (flags & F_MINUS && padd == ' ') /* assgin the extra char to the left of buffer */
        {
            buffer[--ind] = 'x';
            buffer[--ind] = '0';
            if (extra_c)
